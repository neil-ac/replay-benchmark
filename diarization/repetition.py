...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze consecutive n-gram repetitions within speaker segments"
    )
    parser.add_argument(
        "input_file",
        type=Path,
    ...
    parser.add_argument(
        ...
        "--output",
        type=Path,
        help="Output file for repetition analysis (JSON)",
    )
    parser.add_argument(
        "--min-n",
        type=int,
    ...
    # Load and process
    print(f"Loading transcription from: {args.input_file}")
    turns = load_transcription(args.input_file)
    ...
    if stats["most_repeated_phrase"]:
        print("\nMost repeated phrase:")
        mrp = stats["most_repeated_phrase"]
        print(f'  "{mrp["phrase"]}" by {mrp["speaker"]} ({mrp["count"]}x consecutive)')
        print(f'  In segment: "{mrp["segment_text"]}"')

    # Show top repetitions
    if repetitions:
        print("\n" + "-" * 50)
        print("TOP 10 REPETITIONS")
        print("-" * 50)
